...
const RecepcionDashboard: React.FC = () => {
  ...
  const handleAssignPatient = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/patient-assignments`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok) {
        ...
        setSuccessMessage('¡Paciente asignado exitosamente!');
        setShowSuccessToast(true);
      } else {
        const errorData = await response.json();
        setErrorMessage(errorData.message || 'Error al asignar el paciente');
        setShowErrorModal(true);
      }
    } catch (error) {
    ...
    <div className="h-screen bg-gray-50 flex flex-col overflow-hidden">
      ...
      {showAssignPatientModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-900">Asignar Paciente a Médico</h2>
            </div>
            ...
            <form onSubmit={handleAssignPatient} className="p-6 space-y-6">
              <div>
                ...
                <select
                  required
                  value={newAssignment.pacienteId}
                  onChange={(e) => setNewAssignment({...newAssignment, pacienteId: e.target.value})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                >
                  <option value="">Seleccione un paciente</option>
                  {patients.map((patient) => (
                    <option key={patient._id} value={patient._id}>
                      {patient.nombre} {patient.apellido} - C.I: {patient.cedula}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                <select
                  required
                  value={newAssignment.medicoId}
                  onChange={(e) => setNewAssignment({...newAssignment, medicoId: e.target.value})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                >
              ...
              <div>
                ...
                <textarea
                  required
                  rows={3}
                  value={newAssignment.motivoConsulta}
                  onChange={(e) => setNewAssignment({...newAssignment, motivoConsulta: e.target.value})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
              ...
              <div>
                ...
                <select
                  required
                  value={newAssignment.prioridad}
                  onChange={(e) => setNewAssignment({...newAssignment, prioridad: e.target.value as 'alta' | 'media' | 'baja'})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  placeholder="Observaciones adicionales para el médico"
                />
              </div>
